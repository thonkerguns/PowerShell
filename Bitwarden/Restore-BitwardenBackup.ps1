<#
.Synopsis
    Restore-BitwardenBackup.ps1 restores from a Bitwarden backup created by Backup-Bitwarden.ps1

.DESCRIPTION
    Restore-BitwardenBackup.ps1 restores from an encrypted bitwarden backup generated by Backup-Bitwarden.ps1.

.PARAMETER BackupFile
    The full path of the backup file. (e.g., /opt/bitwarden/BitwardenBackup-xx.tar.gpg)
...
.PARAMETER LogFile
    The location where the log file will reside (default location is ./Restore-BitWardenBackup.log)

.EXAMPLE
    ./Restore-BitWardenBackup.ps1 -BackupFile /opt/bitwarden/BitWardenBackup-2022-05-13_00-00-03.tar.gpg -Passwordfile ./password_file

    Restore a backup file located in /opt/bitwarden with the given password file located in the same directory as the script.

.EXAMPLE
    ./Restore-BitwardenBackup.ps1 -BackupFile /opt/bitwarden/BitWardenBackup-2022-05-13_00-00-03.tar.gpg -Passphrase 'Yeehaw_passwords_haha'

    Restore a backup with a given passphrase

.NOTES
    Author - Zack

    THIS SCRIPT WILL STOP & START BITWARDEN, JUST AN FYI BEFORE RUNNING

    The script will only work with provided *.tar.gpg files. 
...
# The Data directories Bitwarden uses
$script:IMPORTANT_DIRECTORIES = "/opt/bitwarden/bwdata/env", "/opt/bitwarden/bwdata/core/attachments", "/opt/bitwarden/bwdata/mssql/data"
#endregion
...
# Check if the given password file exists
if ($PSCmdlet.ParameterSetName -eq "Passwordfile") {
    if (-not (Test-Path -Path $PasswordFile)) {
